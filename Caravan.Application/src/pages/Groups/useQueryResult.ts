...
export function useQueryResult(search: DataTableSearch): PagedViewModel<SocialGroupResponse> {
  const request = {
    pageNumber: search?.start ?? DefaultConsts.FirstPageIndex,
    pageSize: search?.size ?? 10
  } as PagedQueryRequest;

  const { data, isLoading } = useSocialGroupsPagedQuery(request);

  const viewModel: PagedViewModel<SocialGroupResponse> = {
    items: data?.items ?? [],
